...
# ---------------- #
def parse_args():
    parser = argparse.ArgumentParser(description="evaluating predictions from LSTM.")
...
def main():
    ...
    train_medium = invert_medium(medium) if inverter_tag == "on" else medium
    scenario = "inverted" if inverter_tag == "on" else "standard"

    print('**********************************************')
    print('           LSTM PREDICTION EVALUATION         ')
    print('**********************************************\n')
    print(f"dataset: [{mode.upper()}] | data medium: [{medium}] | "
          f"model trained on: [{train_medium}] | inverter: [{inverter_tag}] | "
          f"scaler: [{scaler_tag}] | threshold: [{threshold}]\n")

    # setting paths
    pred_dir = f"/eos/user/l/llimadas/ML_models/LSTM/predictions/{scenario}/{pt_label}"
    results_dir = f"/eos/user/l/llimadas/ML_models/LSTM/results/{scenario}/{pt_label}/{mode}"
    figs_dir = os.path.join(results_dir, "figures")
    os.makedirs(results_dir, exist_ok=True)
    os.makedirs(figs_dir, exist_ok=True)

    # loading predictions
    pred_file = f"{pt_label}_{mode}_predictions_LSTM_train-{train_medium}_data-{medium}_scaler-{scaler_tag}.parquet"
    ...
    roc_base = f"roc_LSTM_{pt_label}_{mode}_train-{train_medium}_data-{medium}_scaler-{scaler_tag}"
    ...
    cm_base = f"cm_LSTM_{pt_label}_{mode}_train-{train_medium}_data-{medium}_scaler-{scaler_tag}"
    ...
    cm_norm_base = f"cm_norm_LSTM_{pt_label}_{mode}_train-{train_medium}_data-{medium}_scaler-{scaler_tag}"
    plt.savefig(os.path.join(figs_dir, f"{cm_norm_base}.pdf"))
    plt.savefig(os.path.join(figs_dir, f"{cm_norm_base}.png"))
    plt.close()

    # computing metrics with bootstrap
    metrics = {
    ...
    metrics_filename = f"metrics_{mode}_LSTM_train-{train_medium}_data-{medium}_scaler-{scaler_tag}.txt"
    ...
    print(f"\nLSTM Evaluation completed. results saved to {results_dir}")
