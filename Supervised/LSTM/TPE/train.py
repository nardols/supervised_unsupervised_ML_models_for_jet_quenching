...
from model.LSTM import LSTM_FC  # model
...
def parse_args():
    parser = argparse.ArgumentParser(description="Sequence preprocessing for jet classification (LSTM).")
    ...
    parser.add_argument("--pt_label", type=str, required=True, help="String of type 'pTmin_pTmax'.")  # must be a string
...
pt_ranges = {pt_interval: (pt_min, pt_max)} # consistent with old code, but here we want to paralelize
...
if __name__ == "__main__":
    ...
    for pt_label in pt_ranges:
        ...
        # Directories
        model_dir = f"/eos/user/l/llimadas/ML_models/LSTM/trained_models/{medium}/{pt_label}" # directory to save the model
        dataset_dir = f"/eos/user/l/llimadas/seq_pre-processor/processed_data/results/{medium}/train/{pt_label}" # directory to access training data
        ...
        # Save model
        torch.save(best_state, os.path.join(model_dir, f"best_model_LSTM_{pt_label}_scaler-{scaler_tag}.pth"))
        
        with open(os.path.join(model_dir, f"best_params_LSTM_{pt_label}_scaler-{scaler_tag}.json"), 'w') as f:
            json.dump(best_params, f, indent=4)
        
        with open(os.path.join(model_dir, f"losses_LSTM_{pt_label}_scaler-{scaler_tag}.json"), 'w') as f:
            json.dump(best_losses, f, indent=4)

        print(f"\nLSTM trained model and hyperparameters saved for pT: {pt_label}")
