...
def parse_args():
    parser = argparse.ArgumentParser(description="Prediction for jet classification using LSTM+Attention.")
...
def predict(model, dataloader, device):
    model.eval()
    probs_all, labels_all, attn_all, lengths_all = [], [], [], []

    for batch_x, batch_y, lengths in dataloader:
        batch_x, lengths = batch_x.to(device), lengths.to(device)

        logits, att_weights = model(batch_x, lengths, return_attention=True)
        ...
        for i, l in enumerate(lengths.cpu().numpy()):
            l = min(l, att_weights.shape[1])
            attn_all.append(att_weights[i, :l].cpu().numpy())

    return np.concatenate(probs_all), np.concatenate(labels_all), np.concatenate(lengths_all), attn_all
...
def main():
    ...
    print('**********************************************')
    print('     LSTM+ATTENTION PREDICTION CALCULATION    ')
    print('**********************************************\n')

    print(f"Dataset: [{mode.upper()}] | Data medium: [{data_medium}] | Model trained on: [{train_medium}] "
          f"| Inverter: [{inverter_tag}] | Scaler: [{scaler_tag}]\n")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model_dir = f"/eos/user/l/llimadas/ML_models/LSTM+Att/trained_models/{train_medium}/{pt_label}"
    model_path = os.path.join(model_dir, f"best_model_LSTM-att_{pt_label}_scaler-{scaler_tag}.pth")
    params_path = os.path.join(model_dir, f"best_params_LSTM-att_{pt_label}.json")
    ...
    output_dir = f"/eos/user/l/llimadas/ML_models/LSTM+Att/predictions/{scenario}/{pt_label}"
    os.makedirs(output_dir, exist_ok=True)

    output_filename = f"{pt_label}_{mode}_predictions_LSTM-att_train-{train_medium}_data-{data_medium}_scaler-{scaler_tag}.parquet"
    ...
    y_proba, y_true, seq_lengths, attn = predict(model, dataloader, device)
    y_pred = (y_proba >= 0.5).astype(int)

    result_df = pd.DataFrame({"True": y_true, "Prob": y_proba, "Length": seq_lengths})
    result_df.to_parquet(output_path, index=False)

    np.save(os.path.join(output_dir, f"attention_{pt_label}_{mode}_train-{train_medium}_data-{data_medium}_scaler-{scaler_tag}.npy"), attn)

    pred_npy = np.column_stack((1 - y_proba, y_proba))  # [N, 2]
    pred_npy_path = os.path.join(
        output_dir,
        f"{pt_label}_{mode}_predictions_LSTM-att_train-{train_medium}_data-{data_medium}_scaler-{scaler_tag}.npy"
    )
    labels_npy_path = os.path.join(output_dir, f"labels_{pt_label}_{mode}_train-{train_medium}_data-{data_medium}_scaler-{scaler_tag}.npy")
    np.save(pred_npy_path, pred_npy)
    np.save(labels_npy_path, y_true)


    print(f"Results saved to: {output_path}")
    print(f"Attention weights saved to: attention_{pt_label}_{mode}_train-{train_medium}_data-{data_medium}_scaler-{scaler_tag}.npy")
    print(f"Prediction probabilities saved to: {pred_npy_path}")
    print(f"Labels saved to: {labels_npy_path}")
